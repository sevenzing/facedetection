...
def detect_faces(cascade, small_im, normal_im, scaleFactor = 1.1):
	...
	'''
	#gray = cv2.cvtColor(small_im, cv2.COLOR_RGB2GRAY)
	gray = normal_im
...
path_to_data = 'data_photo'
for image_name in os.listdir(path_to_data):

	
	print(image_name)
	#im = cv2.imread(path_to_data + '/' + image_name)
	im = np.array(Image.open(path_to_data + '/' + image_name))
	if (im.shape[1] > 1300):
